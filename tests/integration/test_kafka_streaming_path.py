...
def test_kafka_tick_to_feature_snapshot_path(kafka_stack) -> None:
    ...
    cfg.kafka.tick_topic = f"market.ticks.it.{suffix}"
    cfg.kafka.signal_topic = f"signals.raw.it.{suffix}"
    cfg.kafka.consumer_group = f"nexus-alpha-it-{suffix}"
...
def test_kafka_streaming_still_emits_feature_kind(kafka_stack) -> None:
    ...
    try:
        ...
        emitted = 0
        for _ in range(10):
            stats = loop.run_cycle(max_messages=50)
            emitted = stats.emitted_snapshots
            if emitted >= 1:
                break
            time.sleep(0.5)
